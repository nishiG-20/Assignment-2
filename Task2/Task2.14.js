...
//New stock has been purchased. The details of the purchase are provided in a JSON. A sample JSON is
//{id:'B635', purchaseQuantity: 6}
//Define a function that takes the purchase details as a parameter and update the productMaster based on the purchase details. Print the updated productMaster on the console.

function updatePurchaseDetails(prchseDtls){
    let {id,purchaseQuantity}=prchseDtls
    let index=productMaster.findIndex((product)=>{
        return product.id===id
    })
    productMaster[index].quantity= purchaseQuantity

}

updatePurchaseDetails({id:'B635', purchaseQuantity: 6})
